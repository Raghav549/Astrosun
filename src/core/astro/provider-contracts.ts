 ...
 *
 * These contracts deliberately separate scientific consumers from any one
 * ephemeris implementation (JPL DE, SPICE, analytical fallback, etc.).
 */
...
export function assertFiniteState(result: ProviderState): void {
  ...
  if (!values.every(Number.isFinite)) {
    throw new Error(`Provider ${result.id}@${result.version} returned a non-finite state.`);
  }
  if (result.epochJd !== result.epochJd) throw new Error('Provider state epoch is NaN.');
}
...
export function assertProviderAccuracy(actual: ProviderAccuracy, requested: ProviderAccuracy): void {
  ...
  if (rank[actual] < rank[requested]) {
    throw new Error(`Provider accuracy ${actual} is below requested ${requested}.`);
  }
}
